...
public class CarResourceImplTest {
	...
	/**
	 * Getting {@link Car}s from {@link CarResource} when there is no cars on database
	 * should return an empty {@link List} of {@link Car}s.
	 * 
	 * @return void
	 */
	@Test
	 ...
	 */
	@Test
	public void GettingCar_WithExistingCarIdParam_ReturnsResponseWith200CodeAndCarCorrectly () throws CarNotFoundException {
		final long carId = 1;
	...
	/**
	 * Updating {@link Car} with {@link CarResource} with a valid {@link Car} object and existing {@link Car} ID
	 * returns 200 status code and updated {@link Car} 
	 * 
	 * @throws CarNotFoundException when {@link Car} ID doesn't exist 
	 * @return void 
	 */
	@Test
	public void UpdatingCar_ExistingCarAndValidObject_ReturnsResponseWith200CodeAndUpdatedCarCorrectly () throws CarNotFoundException {
		final long carId = 1; 
		final Car car = mockCar("BMW", new Timestamp(new Date().getTime()), "Spain");
		car.setId(carId);
	...
	/**
	 * Updating {@link Car} with {@link CarResource} with a valid {@link Car} object and non existing {@link Car} ID
	 * returns 404 status code and updated {@link Car}
	 * 
	 * @throws CarNotFoundException when {@link Car} ID doesn't exist 
	 * @return void 
	 */
	...
	public void RemovingCar_NonExistingCarId_CarNotFoundExceptionThrown () throws CarNotFoundException {
		...
		// mock CarService
		try {
			when(carService.deleteCar(carId)).thenThrow(new CarNotFoundException("Car with ID "+carId+" not found"));
